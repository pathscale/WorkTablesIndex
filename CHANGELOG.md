...
### Fixed
- Scans select and lock their node atomically under the structural guard: a split committed between choosing a node and locking it migrated not-yet-yielded elements past a backward scan's resume point, silently truncating the scan (present since scans started advancing by cursor lookup; caught by the heavy tier, reproduced deterministically). No API changes.
...
- Ranges with excluded borrowed-key bounds now omit all values for the excluded boundary keys
- Node splitting now uses the current node length instead of its allocation capacity, preventing values from being lost with small maximum node sizes
